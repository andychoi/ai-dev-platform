...
# Stop with SSO overlay first (in case it was started that way)
if [ -f "docker-compose.sso.yml" ]; then
    docker compose -f docker-compose.yml -f docker-compose.sso.yml down 2>/dev/null || true
fi
...
VOLUMES_TO_REMOVE=(
    "coder-poc_postgres-data"
    "coder-poc_authentik-redis"
    "coder-poc_authentik-media"
    "coder-poc_authentik-templates"
    "coder-poc_gitea-data"
    "coder-poc_minio-data"
    "coder-poc_devdb-data"
    "coder-poc_testdb-data"
    "coder-poc_drone-data"
    # Legacy volume names
    "coder-poc-postgres"
    "coder-poc-data"
)
...
# SSO configuration files (regenerated by setup.sh)
FILES_TO_REMOVE=(
    "$POC_DIR/.env.sso"
)
...
# Don't remove .env.combined or docker-compose.sso.yml as they may have manual edits
print_info "Kept: .env.combined, docker-compose.sso.yml (manual config)"
