...
locals {
  ...
  # Authorization: user must be in "java-users" group (Authentik → OIDC → Coder)
  template_authorized = contains(data.coder_workspace_owner.me.groups, "java-users")
}
...
#   1. Admin creates "java-users" group in Authentik
...
#
# Users NOT in "java-users" get a clear error at workspace creation time.
...
resource "null_resource" "template_access_check" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = <<-EOT
        ACCESS DENIED: Java workspace requires membership in the "java-users" group.

        Your groups: ${jsonencode(data.coder_workspace_owner.me.groups)}

        To request access:
          1. Ask your platform admin to add you to the "java-users" group in Authentik
