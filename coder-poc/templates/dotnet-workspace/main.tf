# .NET Workspace Template for Coder
# Docker-based C#/.NET development workspace
...
locals {
  workspace_image = "dotnet-workspace:latest"

  # Authorization: user must be in "dotnet-users" group (Authentik → OIDC → Coder)
  template_authorized = contains(data.coder_workspace_owner.me.groups, "dotnet-users")
}
...
#   1. Admin creates "dotnet-users" group in Authentik
...
#
# Users NOT in "dotnet-users" get a clear error at workspace creation time.
...
resource "null_resource" "template_access_check" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = <<-EOT
        ACCESS DENIED: .NET workspace requires membership in the "dotnet-users" group.

        Your groups: ${jsonencode(data.coder_workspace_owner.me.groups)}

        To request access:
          1. Ask your platform admin to add you to the "dotnet-users" group in Authentik
