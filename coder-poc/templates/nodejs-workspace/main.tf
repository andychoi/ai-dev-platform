# Node.js Workspace Template for Coder
# Docker-based JavaScript/TypeScript development workspace
...
locals {
  workspace_image = "nodejs-workspace:latest"

  # Authorization: user must be in "nodejs-users" group (Authentik → OIDC → Coder)
  template_authorized = contains(data.coder_workspace_owner.me.groups, "nodejs-users")
}
...
#   1. Admin creates "nodejs-users" group in Authentik
...
#
# Users NOT in "nodejs-users" get a clear error at workspace creation time.
...
resource "null_resource" "template_access_check" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = <<-EOT
        ACCESS DENIED: Node.js workspace requires membership in the "nodejs-users" group.

        Your groups: ${jsonencode(data.coder_workspace_owner.me.groups)}

        To request access:
          1. Ask your platform admin to add you to the "nodejs-users" group in Authentik
